...
import com.mocharealm.gaze.glassy.liquid.effect.highlight.HighlightElement
import com.mocharealm.gaze.glassy.core.platform.PlatformVersion
import com.mocharealm.gaze.glassy.core.platform.convertToComposeRenderEffect
import com.mocharealm.gaze.glassy.liquid.effect.shadow.InnerShadow
import com.mocharealm.gaze.glassy.liquid.effect.shadow.InnerShadowElement
import com.mocharealm.gaze.glassy.liquid.effect.shadow.Shadow
import com.mocharealm.gaze.glassy.liquid.effect.shadow.ShadowElement
...
fun Modifier.drawPlainBackdrop(
    ...
    val client = rememberGlassySettingsClient()
    val configuration by client.getSettingsFlow().collectAsState(initial = GlassyConfiguration())
...
private class DrawBackdropElement(
    ...
    val configuration: GlassyConfiguration
...
private class DrawBackdropNode(
    ...
    var configuration: GlassyConfiguration
    ...
    private val effectScope =
        object : BackdropEffectScopeImpl() {
            ...
            override val shape: Shape get() = shapeProvider.innerShape
                ...
                override var configuration: GlassyConfiguration
                get() = this@DrawBackdropNode.configuration
                set(value) { /* Read-only from node perspective, but interface allows set */ }
        }
    ...
    private fun updateEffects() {
        if (PlatformVersion.supportsRenderEffect()) {
            ...
            val platformEffect = effectScope.renderEffect
            if (platformEffect != null) {
                val composeEffect = convertToComposeRenderEffect(platformEffect)
                if (composeEffect != null) {
                    graphicsLayer?.renderEffect = composeEffect
                    println("Glassy: RenderEffect applied successfully, padding=$padding")
                } else {
                    println("Glassy: Failed to convert PlatformRenderEffect to Compose RenderEffect")
                }
            } else {
                graphicsLayer?.renderEffect = null
                println("Glassy: No platform effect to apply")
            }
        } else {
            println("Glassy: RenderEffect not supported on this platform")
        }
    }
